...
def cell_devs_trafico(dim, dir_valfile):
    ...
    #############
    ## Set zones
    with open(dir_valfile, 'r') as f:
        zones = []
        for line in f:
            s = re.search('\((.*),(.*),(.*)\)=(.*)', line)
            x, y, z, val = s.group(1), s.group(2), s.group(3), s.group(4)
            # Agrego como zona=pared a las celdas que tienen valor = 0 (si forma parte de una manzana)
            if val == '1':
                zones.append({'rule': 'pared-rule', 'cells': '(' + ','.join([x,y,z]) + ')'})
    cell_devs.set_zones(zones)
