from root.modulosCDPP.HCPPAtomic.HCPPDEVSAtomic import HCPPDEVSAtomic
        ...
        for extension in ['.h', '.cpp']:
            with open(devsml_cpp_h_directory + pulse_name + extension, 'w+') as f:
                ...
                f.write(self.render_template(template_now, {
                    ...
                    'volume_param': filter(lambda x: x.get('name') == 'volume_param',
                                           p.find('parameters').findall('parameter'))[0].text,
                    'volume_input': filter(lambda x: x.get('name') == 'volume_input',
                                           p.find('parameters').findall('parameter'))[0].text,
                    'firstPulse_input': filter(lambda x: x.get('name') == 'firstPulse_input',
                                               p.find('parameters').findall('parameter'))[0].text,

                    'interval_param': filter(lambda x: x.get('name') == 'interval_param',
                                             p.find('parameters').findall('parameter'))[0].text,
                    'interval_input': filter(lambda x: x.get('name') == 'interval_input',
                                             p.find('parameters').findall('parameter'))[0].text,

                    'dt': filter(lambda x: x.get('name') == 'dt', p.find('parameters').findall('parameter'))[0].text
                }))
