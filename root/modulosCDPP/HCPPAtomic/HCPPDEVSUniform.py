        ...
        for extension in ['.h', '.cpp']:
            with open(devsml_cpp_h_directory + uniform_name + extension, 'w+') as f:
                ...
                f.write(self.render_template(template_now, {
                    ...
                    'dt': filter(lambda x: x.get('name') == 'dt', p.find('parameters').findall('parameter'))[
                        0].text,
                    'min_val':
                        filter(lambda x: x.get('name') == 'min_val', p.find('parameters').findall('parameter'))[
                            0].text,
                    'max_val':
                        filter(lambda x: x.get('name') == 'max_val', p.find('parameters').findall('parameter'))[
                            0].text,
